			...
			.then(() => {
				...
				for (const type of types) {
					const promise = new Promise((resolve, reject) => {
						// @TODO - this doesn't actually work is multiple types are passed in
						let finalIconPath = iconPath || path.join(app.getPath('desktop'), `app.${type}`);
						
						fs.rename(path.join(tempPath, `app.${type}`), iconPath, (err) => {
							if (err) {
								return reject(err);
							}
